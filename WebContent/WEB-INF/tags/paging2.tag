...
<%@ attribute name="pagedLink" required="true" %>
...
<c:if test="${pagedListHolder.pageCount > 1}">
	<ul class="pagination justify-content-end">
		<c:if test="${!pagedListHolder.firstPage}">
			...
			href="${pagedLink}${pagedListHolder.getPage() - 1}">Previous</a></li>
		</c:if>
		<c:if test="${pagedListHolder.firstLinkedPage > 0}">
			<li class="page-item"><a class="page-link" href="${pagedLink}0">1</a></li>
		</c:if>
		...
		<c:if
			...
			<li class="page-item"><a class="page-link"
				href="${pagedLink}${pagedListHolder.getPageCount() - 1}">${pagedListHolder.pageCount}</a></li>
		</c:if>
		<c:if test="${!pagedListHolder.lastPage}">
			<li class="page-item"><a class="page-link"
				href="${pagedLink}${pagedListHolder.getPage() + 1}">Next</a></li>
		</c:if>
	</ul>
</c:if>