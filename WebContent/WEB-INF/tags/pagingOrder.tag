...
<c:if test="${pagedListHolder.pageCount > 1}">
	<ul class="pagination justify-content-end">
		<c:if test="${!pagedListHolder.firstPage}">
			<li class="page-item pagnext"><a class="page-link border border-info"
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() - 1))%>"><i class="fas fa-angle-left fs-4"></i>Previous</a></li>
		</c:if>
		...
		<c:if test="${!pagedListHolder.lastPage}">
			<li class="page-item pagnext"><a class="page-link border border-info"
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() + 1))%>"><i class="fas fa-angle-right fs-4"></i><span>Next</span></a></li>
		</c:if>
	</ul>
</c:if>